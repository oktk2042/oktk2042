# 👋 こんにちは！ / Hello there!

## 🧑‍💻 大木 雅斗（Masato Oki）

富山大学大学院 理工学研究科 数理情報学プログラム 修士課程の大学院生です。  
AI・データサイエンス・マルチエージェント・時系列予測・自然言語処理に関心があります。  
特に、非構造化データを用いた株価予測や、Transformerの解釈可能性向上に取り組んでいます。

I'm a Master's student at the University of Toyama, majoring in Mathematical and Information Sciences.  
My interests lie in **AI, data science, multi-agent systems, time-series forecasting**, and **natural language processing**.  
Currently, I'm focusing on stock price prediction using unstructured data and analyzing Transformer models for interpretability.

---

## 🔬 研究テーマ / Research Topics

- 📈 修士研究 / Master's Research  
  **「非構造化データとAttentionマップ解析を用いた解釈可能なTransformer株価予測モデル」**  
  **"Interpretable Stock Price Forecasting using Transformer and Attention Map Analysis with Unstructured Data"**

- 📊 卒業研究 / Undergraduate Thesis  
  **「実世界データにおけるTransformerを用いた時系列予測の有効性」**  
  **"Time-Series Forecasting with Transformer Models on Real-World Data"**

---

## 🛠 スキルセット / Skills

<div align="center">
 
 <img src="https://skillicons.dev/icons?i=python,cpp,java,matlab,r,mysql,javascript&theme=dark" /><br> 
 <img src="https://skillicons.dev/icons?i=pytorch,tensorflow,sklearn,react,nodejs,flask,fastapi&theme=dark" /><br> 
 <img src="https://skillicons.dev/icons?i=git,github,aws,docker,raspberrypi,vscode,unity&theme=dark" /><br> 

</div>

---

## 💼 インターン / Internship Experience

- 💻 富士通株式会社 / Fujitsu Ltd. (2024年9月～2025年2月 / Sep 2024 – Feb 2025)  
- 🏢 日立製作所 / Hitachi Ltd. (2024年8月～9月 / Aug – Sep 2024)

---

## 📚 資格 / Certifications

- 🧠 G検定 / JDLA Deep Learning for GENERAL  
- 📊 統計検定2級 / Statistics Test Grade 2  
- 💻 基本情報技術者試験 合格 / Fundamental Information Technology Engineer (Passed)  
- 🔜 応用情報技術者試験（予定） / Applied Information Technology Engineer (Planned)  
- 🌍 TOEIC 605点 (L: 325 / R: 280)

---

## 🌱 現在取り組んでいること / Currently Learning

- LLMを活用した予測モデルにおける特徴選択と解釈可能性向上  
  Feature Selection & Interpretability for LLM-based Forecasting  
- 金融テキスト（IR資料、ニュース、SNS）に対するNLP応用  
  NLP for Financial Data (IR, News, Social Media)  
- 統計的検定と評価指標  
  Statistical Testing & Evaluation Metrics

---

## 🎮 趣味・活動 / Hobbies & Activities

- 🎨 趣味: アニメ・漫画・ゲームが大好きです！  
  Hobbies: I love anime, manga, and games!  
- 📚 所属: 大学のアニメ漫画サークルに所属し、企画・広報・組織運営に携わってきました。  
  I’ve also been an active member of my university's anime & manga club, contributing to event planning, PR, and organizational reforms.  
  部誌販売や公認化、SNS戦略を通じて部員を大きく拡大しました。

---

ご覧いただきありがとうございます！今後も継続的に研究成果やプロジェクトを更新していきます。  
Thanks for visiting my profile! I'll keep updating with new projects and research activities. 🚀
